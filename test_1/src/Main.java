...
public class Main {
    ...
    public static void main(String[] args){
        ...
        System.out.println(Integer.toString(userTimezone));

        System.out.println("Введите текущий год в формате unix (10 цифр): ");
        long startOfYear = scanner.nextLong();

        // и текущую дату
        Instant thisMoment = Instant.ofEpochMilli(startOfYear + ( 3600000 * userTimezone));

        LocalDate now = LocalDate.ofEpochDay(Duration.ofMillis(thisMoment.toEpochMilli()).toDays());
        LocalDate firstDay = now.with(firstDayOfYear());

        // нам нужно знать, когда начался этот год
        Instant startOfYearMoment = Instant.ofEpochMilli(firstDay.atStartOfDay().toInstant(ZoneOffset.UTC).toEpochMilli()  + ( 3600000 * userTimezone));

        // и конечную дату нашего графика рассветов и закатов (плюс семь дней)
        Instant lastMoment = Instant.ofEpochMilli(startOfYear + ( 3600000 * userTimezone)).plus(7, ChronoUnit.DAYS);
        ...
        } while ( thisMoment.toEpochMilli() != lastMoment.toEpochMilli() ); // вам нужно вывести график на семь дней
    ...
    private static int dayOfYearFromInstant(Instant startOfYear, Instant time) {
        LocalDate now = LocalDate.ofEpochDay(Duration.ofSeconds(time.getEpochSecond()).toDays());
